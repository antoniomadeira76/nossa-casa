...
7. **Alert colours: grey except real risk.** Informational rows in *Precisa de Si* use a `#A9B4C6` left edge and `#6A7282` icon; **only a limit that has been exceeded is red** (`#CE0002`); **an approaching date is blue** (`state.info` — warranty or prescription expiring, upcoming consultation, calendar to connect). Amber/Warning Yellow is used nowhere — note tiles are neutral, the points pill is neutral, envelope bars step straight from blue to red. *(08/09/2026: the prototype's own `needs` list painted expiring warranties amber, against this rule; the app followed the rule, at the owner's request — no orange in this app.)*
