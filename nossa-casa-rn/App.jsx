...
import Crianca from './src/screens/Crianca';
import { NovaTarefa, NovoEvento, NovoArtigo } from './src/sheets/Criar';
...
function Shell() {
  ...
  const [full, setFull] = useState(null);   // 'equip' | 'saude' | 'gestao' | 'docs' | 'loja'
  const [sheet, setSheet] = useState(null); // 'tarefa' | 'evento' | 'artigo'
  const [kid, setKid] = useState(null);     // nome da criança, quando entra por PIN
  ...
  if (kid) return <Crianca t={t} who={kid} onExit={() => setKid(null)} />;
  if (!user) return <Login t={t} onEnter={setUser} onKid={setKid} />;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.page }}>
      ...
      {/* área de scroll — ou um ecrã de ecrã inteiro, sempre dentro da raiz */}
      {full === 'equip' ? (
        <Equipamentos t={t} onBack={() => setFull(null)} onOpen={() => {}} />
      ) : full === 'saude' ? (
        <Saude t={t} me={user} onBack={() => setFull(null)} onOpen={() => {}} />
      ) : full === 'gestao' ? (
        <Gestao t={t} me={user} onBack={() => setFull(null)} onOpen={() => {}} />
      ) : full === 'docs' ? (
        <Documentacao t={t} onBack={() => setFull(null)} />
      ) : full === 'loja' ? (
        <ModoCompras t={t} onExit={() => setFull(null)} onAdd={() => setSheet('artigo')}
          onDone={(total) => { set(x => ({ registered: x.registered + total })); setFull(null); setTab('dinheiro'); }} />
      ) : (
        <ScrollView style={{ flex: 1, minHeight: 0 }}
          contentContainerStyle={{ padding: 16, gap: S.xl, paddingBottom: S.xl }}>
          <Screen t={t} user={user} go={setTab} openFull={setFull} openSheet={setSheet} />
        </ScrollView>
      )}
      ...
      {perfil ? <Perfil t={t} user={user} onClose={() => setPerfil(false)}
        onSignOut={() => { setPerfil(false); setUser(null); }}
        openFull={(k) => { setPerfil(false); setFull(k); }} /> : null}

      {sheet === 'tarefa' ? <NovaTarefa t={t} me={user} onClose={() => setSheet(null)} /> : null}
      {sheet === 'evento' ? <NovoEvento t={t} me={user} onClose={() => setSheet(null)} /> : null}
      {sheet === 'artigo' ? <NovoArtigo t={t} onClose={() => setSheet(null)} /> : null}
    </View>
  );
}
